...
class StatusChange extends Command
{
    ...
    public function handle()
    {
        ...
        $booking_statushold = PropertyModel::whereIn('booking_status',[2,3])->where('freez','!=',1)->get();
        foreach ($booking_statushold as $asdd)
        {
            ...
            if($asd->booking_status == 4)
            {
               if((\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $asd->cancel_time)->format('Y-m-d H:i') == now()->subMinute(30)->format('Y-m-d H:i'))||( date('Y-m-d H:i', strtotime($asd->cancel_time)) < now()->subMinute(30)->format('Y-m-d H:i') ))
                ...
                {
                    //  dd($asd);
                    $btime = Carbon::parse($asd->booking_time)->format('H:i:s');
                    if(("09:30:00" <= $btime) && ("18:30:00" >= $btime))
                    {
                        $extratime = now()->format('Y-m-d H:i:s');
                    }elseif(("18:30:00" < $btime) && ("23:59:59" >= $btime) ){
                        // $extratime = "11:45:00";
                        // $extratime =now()->format('Y-m-d 11:45:00');
                        $extratime = Carbon::parse($asd->booking_time)->addDay(2)->format('Y-m-d 09:30:00');
                        // $extratime = Carbon::parse(Carbon::tomorrow()->format('Y-m-d 09:30:00'))->format('Y-m-d H:i:s');
                    }
                    elseif(("00:00:00" <= $btime) && ("09:30:00" > $btime))
                    {
                        $extratime = Carbon::parse(Carbon::now()->format('Y-m-d 09:30:00'))->format('Y-m-d H:i:s');
                    }
                    if($extratime <= now()->format('Y-m-d H:i:s')){
                        ...
                        if($asd->waiting_list > 0)
                        {
                            ...
                            $datas= WaitingListMember::where(['scheme_id'=>$asd->scheme_id,'plot_no'=>$asd->plot_no])->first();
                               ...
                               // $datad = WaitingListMember::where($datas->id)->delete();
                                ...
                                ProteryHistory ::create([
                                    ...
                                    'property_id'=>$asd->id,
                                    'action_by'=>null,
                                    'action' => 'Scheme -'.$mailData['scheme_name'].', unit no-'.$mailData['plot_name'].'unit assing from waiting list for customer name '.$datas->owner_name.' with addhar card number ' .$datas->adhar_card_number .'!!',
                                    'past_data' =>json_encode($asd),
                            ...
                            ProteryHistory::create([
                                ...
                                'property_id'=>$asd->id,
                                'action_by'=>null,
                                'action' => 'Unit status change hold to cancel',
                                'past_data' =>json_encode($asd),
                                'new_data' =>json_encode($plot_details),
                                'name' =>null,
                                'addhar_card' =>null
                ...
                if(($asd->booking_time == now()->subHour(2)->subMinute(15)->format('Y-m-d H:i:s'))||( $asd->booking_time < now()->subHour(2)->subMinute(15)->format('Y-m-d H:i:s') )){
                    ...
                    {
                        $dfs = Carbon::parse("18:30:00")->diffInMinutes(Carbon::parse($asd->booking_time)->format('H:i:s'));
                        // $extratime = Carbon::parse(Carbon::tomorrow()->format('Y-m-d 09:30:00'))->addMinutes(135-$dfs)->format('Y-m-d H:i:s');
                        $extratime = Carbon::parse(Carbon::parse($asd->booking_time)->addDay(1)->format('Y-m-d 09:30:00'))->addMinutes(135-$dfs)->format('Y-m-d H:i:s');
                    }elseif(("18:30:00" < $btime) && ("23:59:59" >= $btime) ){
                        // $extratime = "11:45:00";
                        ...
                        if($asd->waiting_list > 0){
                            ...
                            if(!isset($paymentproof)){
                                ...
                                ProteryHistory ::create([
                                    ...
                                    'property_id'=>$asd->id,
                                    'action_by'=>null,
                                    'action' => 'Unit status change booked to cancel',
                                    'past_data' =>json_encode($asd),
                                    'new_data' =>json_encode($plot_details),
                                    'name' =>null,
                                    'addhar_card' =>null
                ...
                $asdnew= PropertyModel::where('id',$asd->id)->first();

                if(($asd->waiting_list > 0) && ($asd->run_auto == 0)){
                    $datas= WaitingListMember::where(['scheme_id'=>$asd->scheme_id,'plot_no'=>$asd->plot_no])->orderBy('id','DESC')->get();
                    if(isset($datas[0])){
                        if(($datas[0]->created_at == now()->subDay(3)->format('Y-m-d H:i:s'))||( $datas[0]->created_at < now()->subDay(3)->format('Y-m-d H:i:s') ))
                        {
                            foreach($datas as $data){
                                ...
                                ProteryHistory ::create([
                                    ...
                                    'property_id'=>$asd->id,
                                    'action_by'=>null,
                                    'action' => 'waiting list remove after 72 hours',
                                ]);
                            }
                        }
                    }
